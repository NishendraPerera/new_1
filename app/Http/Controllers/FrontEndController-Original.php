...
class FrontendController extends Controller
{
    public function index()
    {
        ...
        $articles = Article::select('id', 'title', 'link', 'created_at as date')->orderBy('id', 'desc')->limit(6)->get();

        $videos = Video::select('id', 'title', 'thumbnail_link', 'link', 'created_at as date')->orderBy('id', 'desc')->limit(6)->get();
    ...
    public function single($id)
    {
        $article = Article::join('article_options', 'articles.article_option_id', '=', 'article_options.id')
                        ...
                        ->select('articles.id', 'article_options.name as category', DB::raw("DATE_FORMAT(articles.created_at, '%Y-%m-%d') as date"), DB::raw("DATE_FORMAT(articles.created_at, '%Y-%m-%d') as date"), 'articles.title', 'articles.content', 'users.name as user', 'articles.link')->orderBy('id', 'desc')->where('articles.id', $id)->first();
    ...
    public function articles()
    {
        ...
        $articles = Article::select('id', 'title', 'link',DB::raw("DATE_FORMAT(articles.created_at, '%Y-%m-%d') as date"))->orderBy('id', 'desc')->paginate(2);

        foreach($articles AS $data){
            $data->date = $this->get_time_ago(strtotime($data->date));
        }

        return view('frontend.articles', compact('article', 'articles'));
    }
